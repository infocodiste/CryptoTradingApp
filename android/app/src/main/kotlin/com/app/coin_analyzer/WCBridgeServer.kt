...
class WCBridgeServer(moshi: Moshi) : WebSocketServer(InetSocketAddress(PORT)) {
    ...
    override fun onMessage(conn: WebSocket?, message: String?) {
        ...
        try {
            ...
            message?.also {
                Log.d("#####", "Message: $it")
                val parse = it.replace(",\"silent\":true", ",\"silent\":\"true\"")
                Log.d("#####", "Message Parse: $parse")
                val msg = adapter.fromJson(parse) ?: error("Invalid message")
